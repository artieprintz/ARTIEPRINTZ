...
import { ArrowRight, Box, Image as ImageIcon, Shirt, Layout, BookOpen, ShoppingBag, Plus } from 'lucide-react';
  ...
  return (
    <div className="bg-[#080808] relative overflow-hidden">
      ...
      <section className="py-32 px-10 max-w-7xl mx-auto relative">
        ...
        <div className="relative z-10">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-20 gap-8">
             ...
             <Link to="/shop" className="text-[11px] uppercase tracking-[0.2em] font-medium flex items-center group border-b border-zinc-800 pb-2 hover:text-[#FF6B4A] hover:border-[#FF6B4A] transition-all">
      ...
      <section className="py-32 relative overflow-hidden border-y border-zinc-800">
        ...
        <div className="flex space-x-8 overflow-x-auto pb-12 no-scrollbar px-10 md:px-[calc((100vw-80rem)/2)] items-start">
          {CATEGORIES.map((cat) => (
            <Link 
              key={cat.id}
              to={`/shop?category=${cat.id}`}
              className="flex-shrink-0 group relative w-[350px] md:w-[450px] aspect-[4/5] overflow-hidden bg-zinc-800 border border-zinc-700 shadow-2xl"
            >
              <img 
                src={cat.image} 
                ...
                alt={cat.label} 
              />
              <div className="absolute inset-0 bg-gradient-to-t from-[#080808] via-transparent to-transparent opacity-80 group-hover:opacity-40 transition-opacity" />
              <div className="absolute inset-0 p-12 flex flex-col justify-end">
                <span className="text-5xl font-black tracking-tighter italic uppercase">{cat.label}</span>
                <div className="mt-4 w-12 h-px bg-brand-accent transition-all duration-500 group-hover:w-24" />
                <span className="text-[10px] uppercase tracking-[0.3em] opacity-40 mt-6 block group-hover:opacity-100 transform group-hover:translate-x-2 transition-all">Shop now ↗</span>
              </div>
            </Link>
          ))}
        </div>
      ...
      <section id="how-to-order" className="py-40 px-10 relative overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-24 items-center relative z-10">
           <motion.div
             ...
             viewport={{ once: true }}
           >
              <h2 className="text-zinc-600 text-[10px] font-bold uppercase tracking-[0.5em] mb-8">GUIDE</h2>
              ...
              <p className="text-zinc-400 text-lg font-light leading-relaxed mb-16 max-w-md italic font-medium">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-12 mb-16">
                {[
                  ...
                  <div key={i} className="group">
                    <div className="w-10 h-10 border border-zinc-800 flex items-center justify-center mb-6 group-hover:border-brand-accent transition-colors">
                      <step.icon className="w-4 h-4 text-zinc-500 group-hover:text-brand-accent transition-colors" />
                    </div>
                    <h4 className="text-[10px] font-bold uppercase tracking-widest mb-2 text-white italic">{step.title}</h4>
                    <p className="text-zinc-500 text-[10px] uppercase tracking-tighter leading-tight font-medium">{step.desc}</p>
                  </div>
                ))}
              </div>
      ...
      <section className="py-40 px-10 bg-gradient-to-b from-[#080808] to-zinc-900/20">
        <div className="max-w-4xl mx-auto text-center">
            ...
            <motion.div 
               ...
               viewport={{ once: true }}
               className="mb-40 p-12 border border-zinc-900 bg-zinc-950/50 backdrop-blur-sm"
            >
               ...
               <Link to="/commercial" className="text-[10px] font-black uppercase tracking-[0.3em] border-b border-brand-accent pb-2 hover:text-brand-accent transition-colors">Learn More ↗</Link>
            </motion.div>
            ...
            <div className="space-y-20">
               ...
               >
                 ...
                 <div className="flex items-center justify-center space-x-4">
                    ...
                    <span className="text-[11px] font-black tracking-[0.4em] text-zinc-500 uppercase">— OUR MISSION</span>
                    <div className="w-8 h-px bg-zinc-800" />
                 </div>
               </motion.div>
            </div>
      ...
      <Link to="/cart" className="fixed bottom-8 right-8 z-50 bg-zinc-900 text-white px-4 py-3 font-black uppercase tracking-[0.2em] hover:bg-white hover:text-black transition-colors shadow-2xl italic flex items-center space-x-3">
