  ...
  if (items.length === 0) {
    return (
      <div className="min-h-screen pt-40 px-10 max-w-7xl mx-auto text-center bg-[#080808]">
         ...
         <h1 className="text-6xl md:text-8xl font-black tracking-tighter italic mb-8 uppercase text-white">YOUR CART IS EMPTY</h1>
         <p className="text-zinc-300 text-base mb-16 max-w-sm mx-auto font-medium tracking-tight">You haven't added anything to your cart yet. Browse our shop to find something you like.</p>
         <Link to="/shop" className="bg-white text-black px-12 py-6 text-[11px] font-black uppercase tracking-[0.3em] inline-block hover:bg-brand-accent hover:text-white transition-all transform hover:-translate-y-1 italic cursor-pointer">
            Start Shopping
         </Link>
      </div>
    );
      ...
      <div className="max-w-7xl mx-auto">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-16 gap-8">
           <div>
              ...
              <p className="text-zinc-350 text-[11px] uppercase tracking-[0.4em] font-medium">{items.length} items in your cart</p>
           </div>
           <Link to="/shop" className="text-[10px] font-black uppercase tracking-[0.3em] flex items-center text-zinc-300 hover:text-white transition-colors border border-zinc-700 px-6 py-3 cursor-pointer">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-16">
          <div className="lg:col-span-8 space-y-6">
             ...
             {suggestions.length > 0 && (
                <motion.div 
                  ...
                  className="p-8 border border-brand-accent/40 bg-brand-accent/5 mb-8"
                >
                  ...
                  {suggestions.map((s, i) => (
                    <p key={i} className="text-zinc-300 text-sm font-medium italic leading-relaxed">
                  ...
                  >
                    <div className="absolute top-0 right-0 p-4 opacity-5 group-hover:opacity-20 transition-opacity">
                     ...
                     <div className="md:ml-10 flex-1 text-center md:text-left z-10">
                        <h3 className="text-4xl font-bold tracking-tighter italic uppercase group-hover:text-brand-accent transition-colors text-white">{item.name}</h3>
                        <div className="mt-4 space-y-2 text-[10px] uppercase tracking-[0.2em] text-zinc-400 font-bold">
                        ...
                        <div className="mt-10 flex items-center justify-center md:justify-start space-x-8">
                           <div className="flex items-center border border-zinc-700 px-6 py-3 space-x-6 bg-[#080808]">
                              <button onClick={() => updateQuantity(item.id, item.quantity - 1)} className="text-zinc-400 hover:text-white transition-colors cursor-pointer"><Minus className="w-3 h-3" /></button>
                              <span className="text-[10px] font-black w-4 text-center text-white">{item.quantity}</span>
                              <button onClick={() => updateQuantity(item.id, item.quantity + 1)} className="text-zinc-400 hover:text-white transition-colors cursor-pointer"><Plus className="w-3 h-3" /></button>
                           </div>
                           <button 
                             onClick={() => removeItem(item.id)}
                             className="text-zinc-400 hover:text-brand-accent transition-colors flex items-center space-x-2 text-[10px] uppercase font-bold tracking-widest cursor-pointer"
                           >
                     ...
                     <div className="mt-8 md:mt-0 font-mono text-xl text-zinc-300 font-bold">
          ...
          <div className="lg:col-span-4">
             <div className="bg-zinc-900 border border-zinc-800 p-12 sticky top-32 shadow-2xl">
                ...
                <h3 className="text-4xl font-black italic uppercase mb-12 border-b border-zinc-800 pb-8 text-white">Order Total</h3>
                ...
                <button 
                  ...
                  className="w-full bg-white text-black py-8 mt-16 text-[14px] font-black uppercase tracking-[0.3em] hover:bg-brand-accent hover:text-white transition-all transform hover:-translate-y-1 shadow-2xl flex items-center justify-center space-x-3 italic cursor-pointer"
                >
                ...
                <div className="lg:hidden fixed bottom-0 left-0 right-0 p-6 bg-[#080808]/80 backdrop-blur-xl border-t border-zinc-800 z-50">
                   <button 
                     onClick={() => navigate('/checkout')}
                     className="w-full bg-brand-accent text-white py-6 text-[12px] font-black uppercase tracking-[0.4em] shadow-2xl flex items-center justify-center space-x-3 italic active:scale-95 transition-transform cursor-pointer"
                   >
                ...
                <p className="mt-10 text-[9px] text-center uppercase tracking-[0.2em] text-zinc-400 leading-relaxed italic font-medium">
